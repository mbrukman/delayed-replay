...
class ProxyRequestHandler(SimpleHTTPServer.SimpleHTTPRequestHandler):
  ...
  def __getTargetUrl(self, target_url):
    ...
    self.send_header(HTTP_HEADER_CONTENT_TYPE, headers[HTTP_HEADER_CONTENT_TYPE])
    self.end_headers()
...
def main(argv):
  # TODO: use optparse, add a --port flag with a default, use a built-in validator.
  if len(argv) < 2:
  ...
  try:
    # Only accept connections from localhost for security as this is a proxy.
